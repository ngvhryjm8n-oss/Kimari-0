...
export const DIZIONARIO = {
  ...
  /* --------------------------------- conferma senza niente da votare (fix 1) */
  'Non c’è niente da votare: data e luogo li hai già scelti tu. Confermando, il piano entra in calendario e chi ha il link vede che è deciso.': {
    en: 'There’s nothing to vote on: you already picked the date and the place. Confirm and the plan goes in your calendar, and whoever has the link sees it’s settled.',
    es: 'No hay nada que votar: la fecha y el sitio ya los has elegido tú. Al confirmar, el plan entra en el calendario y quien tenga el enlace ve que está decidido.',
    de: 'Es gibt nichts abzustimmen: Termin und Ort hast du schon festgelegt. Mit dem Bestätigen kommt der Plan in den Kalender, und wer den Link hat, sieht, dass er steht.',
    ja: '投票することはありません。日時も場所もすでにあなたが決めています。確定するとカレンダーに入り、リンクを持つ人には決定済みと表示されます。',
    fr: 'Il n’y a rien à voter : la date et le lieu, tu les as déjà choisis. En confirmant, le plan entre dans le calendrier et ceux qui ont le lien voient que c’est décidé.' },
